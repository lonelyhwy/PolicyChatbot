from blocks.interface import create_demo
from utils.pdfchatbot import PDFChatBot
...
with demo:
    ...
    submit_button.click(pdf_chatbot.add_text, inputs=[chat_history, txt], outputs=[chat_history], queue=False).\
        success(pdf_chatbot.generate_response, inputs=[chat_history, txt, uploaded_pdf], outputs=[chat_history, txt], queue=True).\
